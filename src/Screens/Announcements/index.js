...
const renderItem = ({item}) => {
  return (
    <View style={styles.cell}>
      ...
      <Text style={styles.cellDate}>{`${Moment(item.startDate).format(
        'MMM Do YYYY, h:mm:ss a'
      )} - ${Moment(item.endDate).format('MMM Do YYYY, h:mm:ss a')}`}</Text>
      <Text style={styles.cellDescription}>
        {
          'Thêm ngày cho chuyến đi của bạn để nhận thông tin về chính sách hủy cho đặt phòng này. Chủ nhà không cho phép mang theo thú cưng, tổ chức tiệc tùng hoặc hút thuốc.'
        }
      </Text>
...
const styles = StyleSheet.create({
  ...
  cellImage: {
    ...
    height:
      ((Dimensions.get('window').width - LIST_PADDING_HORIZONTAL * 2) / 5) * 3,
    marginBottom: 5,
